...
logger = logging.getLogger("ccpredicting")
logger.setLevel(logging.INFO)

from final_model.predict_api import predict_one  # backend now returns IG
...
.card { border: 1px solid rgba(0,0,0,.08); border-radius: 16px; padding: 1rem 1rem; background: #ffffff; }
.card-title { font-weight: 780; margin-bottom: .35rem; }
.hr-soft { height: 1px; background: rgba(0,0,0,.06); margin: .85rem 0; }

.risk-banner { border-radius: 14px; padding: .85rem .95rem; border: 1px solid rgba(0,0,0,.08); }
...
.stButton>button { border-radius: 12px !important; padding: .60rem 1.00rem !important; font-weight: 700 !important; }

/* ---- Input summary 2-row grid ---- */
.summary-grid { border: 1px solid rgba(0,0,0,.08); border-radius: 14px; overflow: hidden; background: #fff; }
...
.summary-k { font-size: 0.82rem; color: rgba(0,0,0,.55); margin-bottom: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.summary-v { font-size: 1.05rem; font-weight: 760; color: rgba(0,0,0,.90); }
...
# -------------------------
# Feature order (must match your backend preprocess)
# -------------------------
FEATURE_ORDER = [
...
def risk_band(prob: float) -> str:
    ...
    if prob < 0.30: return "Intermediate"
    return "High"
...
def render_ig(ig_payload):
    if not ig_payload:
        st.info("IG explanation was not returned by the model.")
        return
    if isinstance(ig_payload, dict) and ("positive" in ig_payload or "negative" in ig_payload):
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("**Factors increasing risk**")
            for r in (ig_payload.get("positive") or [])[:12]:
                st.write(f"- {r}")
        with c2:
            st.markdown("**Factors decreasing risk**")
            for r in (ig_payload.get("negative") or [])[:12]:
                st.write(f"- {r}")
        return
    if isinstance(ig_payload, list):
        for r in ig_payload[:12]:
            st.write(f"- {r}")
        return
    st.write(str(ig_payload))
...
with st.sidebar:
    ...
    menopausal_status = st.selectbox(VAR_DICT["menopausal_status"]["label"], [0, 1], format_func=lambda x: "Premenopausal" if x == 0 else "Postmenopausal")
    ...
    child_alive = st.selectbox(VAR_DICT["child_alive"]["label"], [0, 1], format_func=lambda x: "No" if x == 0 else "Yes")

    st.divider()
    st.subheader("HPV status")
    HPV_overall = st.selectbox(VAR_DICT["HPV_overall"]["label"], [0, 1], format_func=lambda x: "Negative" if x == 0 else "Positive")
    HPV16 = st.selectbox(VAR_DICT["HPV16"]["label"], [0, 1], format_func=lambda x: "Negative" if x == 0 else "Positive")
    HPV18 = st.selectbox(VAR_DICT["HPV18"]["label"], [0, 1], format_func=lambda x: "Negative" if x == 0 else "Positive")
    HPV_other_hr = st.selectbox(VAR_DICT["HPV_other_hr"]["label"], [0, 1], format_func=lambda x: "Negative" if x == 0 else "Positive")
    ...
    iodine_negative = st.selectbox(VAR_DICT["iodine_negative"]["label"], [0, 1], format_func=lambda x: "No" if x == 0 else "Yes")
    atypical_vessels = st.selectbox(VAR_DICT["atypical_vessels"]["label"], [0, 1], format_func=lambda x: "No" if x == 0 else "Yes")
...
with tab_single:
        ...
        if run_btn:
            with st.spinner("Computing risk..."):
                try:
                    ...
                    # Do NOT expose backend details to users; log only.
    ...
    if st.session_state["last_pred"] is None:
        ...
        ig_payload = st.session_state["last_pred"].get("ig", None)
        render_ig(ig_payload)
            ...
            if run_batch:
                ...
                for i, row in df.iterrows():
                    ...
                    try:
                        ...
                        p = float(pred.get("prob"))
                        out_rows.append({"row": i, "probability": p, "risk_band": risk_band(p)})
                    except Exception:
...
with tab_dict:
    ...
    rows = [{"Variable": k, "Label": VAR_DICT[k]["label"], "Description": VAR_DICT[k]["desc"]} for k in FEATURE_ORDER]
